...
public class OrderTest {
    ...
    public void acceptOrderTest() {
        ...
        Response response1 = orderAPIHelper.acceptOrder(orderId, courierId);
    ...
    public void acceptOrderWithWrongCourierIdTest() {
        ...
        Response response1 = orderAPIHelper.acceptOrder(orderId, Integer.MAX_VALUE);
        response1.then().assertThat().body("message", equalTo("Курьера с таким id не существует"))
                .and()
                .statusCode(404);
        orderAPIHelper.cancelOrder(track);
    }
    ...
    public void acceptOrderWithoutCourierIdTest() {
        ...
        Response response1 = orderAPIHelper.acceptWithoutCourierId(orderId);
        response1.then().assertThat().body("message", equalTo("Недостаточно данных для поиска"))
                .and()
                .statusCode(400);
        orderAPIHelper.cancelOrder(track);
    }
    ...
    @Test
    @DisplayName("Check status code of Order by track")
    public void getOrderByTrackTest() {
        ...
        JsonObject rootObject = jsonParser.parse(json).getAsJsonObject();
        int track = rootObject.get("track").getAsInt();
        Response response1 = orderAPIHelper.getOrderByTrack(track);
        response1.then().assertThat().body("order", notNullValue())
                .and()
                .statusCode(200);
        orderAPIHelper.cancelOrder(track);
    }
    ...
    public void getOrderWithoutTrackTest() {
        Response response1 = orderAPIHelper.getOrderWithoutTrack();
        response1.then().assertThat().body("message", equalTo("Недостаточно данных для поиска"))
                .and()
                .statusCode(400);
    }
    ...
    public void getOrderWrongTrackTest() {
        Response response1 = orderAPIHelper.getOrderByTrack(Integer.MAX_VALUE);
        response1.then().assertThat().body("message", equalTo("Заказ не найден"))
                .and()
                .statusCode(404);
    }
}